...
def read_county_borders(file_path):
    ...
    """
    try:
        # Try to read the file as a shapefile
        sf = shapefile.Reader(file_path)

        # Extract county boundaries and properties
        counties = {
            'names': [],
            'polygons': [],
            'centroids': []
        ...
        # Get county names from records
        for record in sf.records():
            counties['names'].append(record[0] if record else "Unknown")
